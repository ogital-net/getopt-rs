...
pub struct Getopt {
    args: Vec<String>,
    c_args: Vec<CString>,
    c_args_ptrs: Vec<*mut libc::c_char>,
    optstring: CString,
}
...
impl Getopt {
    ...
    /// ```
    pub fn new<A, I, O>(args: A, optstring: O, log_err: bool) -> Self
    where
        ...
        O: Into<Vec<u8>>,
    {
        ...
        let c_args: Vec<CString> = args
            ...
            .collect();
        let c_args_ptrs: Vec<*mut libc::c_char> = c_args
            .iter()
    ...
    /// Creates a new Getopt parser using the program's actual command-line arguments.
    ///
    /// This is a convenience wrapper around `new()` that uses `std::env::args()`.
    ///
    /// # Arguments
    /// * `optstring` - A string containing the valid options
    ///
    ...
    /// ```
    /// use getopt_rs::Getopt;
    /// let opts = Getopt::from_args("f:");
    /// ```
    pub fn from_args<O>(optstring: O) -> Self
    where
        O: Into<Vec<u8>>,
    {
        Self::new(std::env::args(), optstring, true)
    }
    ...
    /// ```
    /// use getopt_rs::Getopt;
    /// let opts = Getopt::new(vec!["program", "-f", "file.txt", "arg1", "arg2"], "f:", true);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let arg = unsafe {
            if optarg.is_null() {
                ...
                // SAFETY: the provided args were verified to be valid UTF-8
